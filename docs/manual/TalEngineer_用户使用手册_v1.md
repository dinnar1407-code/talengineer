...
档案里工程师自己写的技能只是参考；**平台认证**是真刀真枪考出来的（AI 出题 + 限时 + 人工复核），并且是指派的硬门禁。雇主看到认证徽章时，可以放心它背后有考核记录。部分工程师还多一层 **🛡️ 已背调**徽章（人工核验通过、365 天内有效）——与认证徽章是两回事：认证考的是技术能力，背调核实的是身份与背景，两者可以叠加展示。
...
全站支持 **9 种语言**（英/中/西/越/印地/法/德/日/韩，导航栏切换）和**深浅色主题**（🌙/☀️ 按钮）。绝大多数页面（含控制台、发布需求、认证中心、Playbook 文章）已是九语全量；个别登录态深度页面仍在补齐中。
...
- **War Room**（/warroom）：实时聊天 + **AI 自动翻译**（中文雇主与西语/英语工程师各说各话、各看各文），另有 AI 日报与质检照片分析。
...
雇主可在 /pools 建立私有的"人才池"——按认证方向、等级等条件圈定一批持证工程师，形成自己的常用候选人清单，反复用工时不用每次都重新筛选。池子仅雇主自己可见。

### 4.11 工程师覆盖地图

/coverage 按地区实时聚合展示平台工程师的分布——数量、当前可接单量、认证档位。发需求前先看看目标地区有没有现成的人才储备，帮你判断是走本地招募还是跨境调配。

### 4.12 AI 助手

首页、发布需求页（/talent）、控制台（登录后）、Finance 页右下角有个对话气泡——点开是平台的 AI 助手，九语跟随界面语言。**未登录也能问**：查费率基准、查认证方向说明、搜工程师。**登录后**（雇主/工程师/管理员视角不同）能多做几件事：查看自己的项目与里程碑状态、查认证考试信息、拿到匹配推荐；雇主用自然语言描述需求，AI 会生成一张**「需求草稿」卡片**，你确认无误、手动点「确认发布」才会真正创建项目——**AI 不会替你自动发布任何东西**，草稿到发布之间永远有一次人工点击。

### 4.13 已背调 🛡️（雇主视角）

部分工程师主页会显示「已背调」徽章（区别于左侧提到的技术认证徽章）——这是身份与背景的人工核验，有效期 365 天。/talent 浏览页可以勾选「只看已背调」筛选。这项核验目前是人工流程，覆盖率会随时间增长，暂时没有覆盖不代表工程师有问题。
...
**Q：支持哪些语言？**
界面 9 种语言；War Room 聊天自动翻译，双方各看各的语言。
