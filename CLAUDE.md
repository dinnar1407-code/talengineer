...
> **[IMPORTANT] 所有代码生成必须严格遵守 `ARCHITECTURE.md`（架构宪法）中定义的红线约束**——单入口拓扑、Socket.IO 监听器生命周期与零状态（S1/S2）、Stripe raw-body 验签与事件零信任（W1）、裸 req.body 的 Zod 禁令（W2）、日志 PII 脱敏（P1）。动手改代码前先读它；改动与宪法冲突时，先修宪并说明理由。
