...
async function doReplayAll() {
  ...
  for (const op of ordered) {
    // qc-image 专走 socket 重发路径（warroom 回网时检测 pending 后经 socket uploadQualityImage 重发 + markDone）。
    // 这里若按 HTTP 重放，会打到占位 url 得 404 → 被判 'fail' 终态误杀，离线拍的图就丢了。跳过它，
    // 留在 pending 里等 warroom 处理；remaining 计数自然把它算进去。
    if (op.type === 'qc-image') continue;
