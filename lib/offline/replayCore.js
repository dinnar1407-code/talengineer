...
// - 401/403 是鉴权失败：离线期间入队时用的 token 可能已过期，用户重登后带新 token 重放就能成功，
//   所以也归 'retry'——绝不能把合法的离线写当业务拒绝静默丢掉
// - 其余 4xx（400 参数错、404 不存在、409 冲突、422 校验失败等）是业务拒绝 → 'fail'（终态，重发也没用）
function classifyFailure(status) {
  if (!status || status >= 500 || status === 429 || status === 401 || status === 403) return 'retry';
  return 'fail';
}
